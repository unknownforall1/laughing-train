...
async def pm_AutoFilter(client, msg, pmspoll=False):  
    ...
    if SHORT_URL and SHORT_API:          
        ...
        if SINGLE_BUTTON:
            btn = [[InlineKeyboardButton(text=f"[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('http') and not x.startswith('t.me') and not x.startswith('www.'), file.file_name.split()))}", callback_data=f'{pre}#{file.file_id}')] for file in files ]
        else:
